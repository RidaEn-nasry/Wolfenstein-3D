...
/*   Updated: 2022/07/27 09:50:09 by ren-nasr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	project3d(t_map	*map)
{
	...
	while (i < WIDTH)
	{
		...
		get_txtr(map, map->rndr->wall->arr_angl[i], i);
...
t_wall	*cast_v(t_map *map, char **which_map)
{
	t_vector	inters;
	t_vector	nwall;
	t_wall		*tmp;
	...
	inters = get_vert_inters(map, tmp, start);
	nwall.x = inters.x;
	nwall.y = inters.y;
	...
	nwall.y >= 0 && nwall.y <= (CELL_SIZE * map->h))
		if (check_inters_v(map, &nwall, tmp, which_map))
			break ;
	if (!map->txtr->found_v)
...
t_wall	*cast_h(t_map *map, char **which_map)
{
	t_vector	nwall;
	t_wall		*tmp;
	t_vector	start;
	...
	start.y = map->rndr->pvec->y + (PLY_SIZE / 2);
	map->rndr->wall->ray_angl = normalize_ang(map->rndr->wall->ray_angl);
	inter = get_horiz_inters(map, tmp, start);
	nwall.x = inter.x;
	nwall.y = inter.y;
	...
	while (nwall.x >= 0 && nwall.x <= (CELL_SIZE * map->w) && \
	nwall.y >= 0 && nwall.y <= (CELL_SIZE * map->h))
		if (check_inters_h(map, &nwall, tmp, which_map))
			break ;
	if (!map->txtr->found_h)
		set_biggest_dist(tmp);
	return (tmp);
...
void	cast(t_map *map, char **which_map)
{	
	t_wall		*wall_h;
	t_wall		*wall_v;

	wall_h = cast_h(map, which_map);
	wall_v = cast_v(map, which_map);
	get_smaller_dist(map, wall_h, wall_v);
}
...
void	cast_rays(t_map *map, char **which_map)
{
	...
	i = 0;
	map->rndr->wall->ray_angl = map->rndr->rot_angl - (map->rndr->fov / 2);
	while (i < WIDTH)
	{
		...
		map->rndr->wall->ray_angl += (map->rndr->fov / WIDTH);
		i++;
	}
}
